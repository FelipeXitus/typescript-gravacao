...
const schemaBodyAdotante: yup.ObjectSchema<Omit<TipoRequestBodyAdotante, "endereco">> = yup.object().shape({
  ...
  cpf: yup.string().defined().required("O CPF é obrigatório").matches(/(\d{3})(\d{3})(\d{3})(\d{2})/, "O CPF deve estar no formato XXXXXXXXXXX"),
...
export const middlewareValidadorBodyAdotante = async (req: Request, res: Response, next: NextFunction) => {
